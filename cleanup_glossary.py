...
def clean_glossary(
    glossary: dict, excluded_terms: set, min_occurrences: int = 1
) -> dict:
...
def main():
    ...
    parser.add_argument(
        "book_directory", help="Path to book directory containing glossary.txt"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Minimum occurrences to keep term (not implemented yet)",
    )

    args = parser.parse_args()

    book_dir = Path(args.book_directory).resolve()
    ...
    if not glossary_path.exists():
        print(f"No glossary.txt found in {book_dir}")
        sys.exit(1)
    ...
    cleaned_glossary, removed_terms = clean_glossary(
        original_glossary, excluded_terms, args.min_occurrences
    )
    ...
    if args.dry_run:
        ...
        if len(removed_terms) > 0:
            ...
            if response.lower().startswith("y"):
                save_glossary(
                    cleaned_glossary, glossary_path, backup=not args.no_backup
                )
